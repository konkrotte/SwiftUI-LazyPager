...
public class ViewDataProvider<Content: View, DataCollecton: RandomAccessCollection, Element>:
    UIViewController, ViewLoader
where DataCollecton.Index == Int, DataCollecton.Element == Element {

    var viewLoader: (Element, Int) -> Content
    ...
    init(
        ...
        viewLoader: @escaping (Element, Int) -> Content
    ) {
    ...
    func loadView(at index: Int) -> ZoomableView<Element, Content>? {
        ...
        let hostingController = UIHostingController(rootView: viewLoader(dta, index))
        return ZoomableView(
            hostingController: hostingController, index: index, data: dta, config: config)
    }
    ...
    func updateHostedView(for zoomableView: ZoomableView<Element, Content>) {
        ...
        zoomableView.hostingController.rootView = viewLoader(dta, zoomableView.index)
    }
