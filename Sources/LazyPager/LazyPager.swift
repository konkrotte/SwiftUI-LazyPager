...
where DataCollecton.Index == Int, DataCollecton.Element == Element {
    private var viewLoader: (Element, Int) -> Content
    ...
    public init(
        ...
        self.viewLoader = { element, _ in content(element) }
    ...
    public init(
        ...
        page: Binding<Int>? = nil,
        direction: Direction = .horizontal,
        @ViewBuilder content: @escaping (Element, Int) -> Content
    ) {
        self.data = data
        self.providedPage = page
...
extension LazyPager {
    public func onDismiss(
        backgroundOpacity: Binding<CGFloat>? = nil, _ callback: @escaping () -> Void
    ) -> LazyPager {
    ...
    public func onDrag(_ callback: @escaping () -> Void) -> LazyPager {
        var this = self
        this.config.dragCallback = callback
        return this
    }

    public func pageSpacing(_ spacing: CGFloat) -> LazyPager {
        var this = self
    ...
    public func shouldLoadMore(
        on: LoadMore = .lastElement(minus: 3), _ callback: @escaping () -> Void
    ) -> LazyPager {
        var this = self
    ...
    public func zoomable(min: CGFloat, max: CGFloat, doubleTapGesture: DoubleTap = .scale(0.5))
        -> LazyPager
    {
        var this = self
    ...
    public func absoluteContentPosition(_ absoluteContentPosition: Binding<CGFloat>? = nil)
        -> LazyPager
    {
        guard config.direction == .horizontal else {
            return self
...
extension LazyPager: UIViewControllerRepresentable {
    ...
    public func updateUIViewController(
        _ uiViewController: ViewDataProvider<Content, DataCollecton, Element>, context: Context
    ) {
        ...
        if page.wrappedValue != uiViewController.pagerView.currentIndex {
            ...
            uiViewController.goToPage(
                page.wrappedValue, animated: context.transaction.animation != nil)
        }
